...
export async function updateUsernameHandler(req: any, res: any, next: any) {
  try {
    const { user_id, username } = req.body ?? {}
    if (!user_id || typeof user_id !== 'string') return res.status(400).json({ error: 'user_id required' })
    if (!username || typeof username !== 'string' || username.length > 30) return res.status(400).json({ error: 'valid username required' })

    const { updateLeaderboardUsername } = await import('../services/leaderboardService')
    await updateLeaderboardUsername(user_id, username.trim())
